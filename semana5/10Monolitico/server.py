...
db = {
    ...
    2: {
        ...
        "content": "Bienvenidos a mi blog Aqui hay otra publicación.",
    },
}
    ...
    def do_PUT(self):
        ...
        if self.path.startswith("/posts/"):
            ...
            if post_id in db:
                ...
                post_data = self.rfile.read(content_length)
                data = json.loads(post_data.decode("utf-8"))
                db[post_id]["title"] = data.get("title", [db[post_id]["title"]])[0]
                db[post_id]["content"] = data.get("content", [db[post_id]["content"]])[
                    0
                ]
                self.send_response(200)
                self.send_header("Content-type", "application/json")
